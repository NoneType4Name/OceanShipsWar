...
def update_game(ver):
    global run
    mb = 0
    file = requests.get(f'https://github.com/AlexKim0710/OceanShipsWar/releases/latest/download/OceanShipsWar.exe',
                        stream=True)
    with open(f'OceanShipsWar.exe', 'wb') as f:
        for chunk in file.iter_content(1024 * 1024):
            f.write(chunk)
            mb += 1
    subprocess.Popen(f'OceanShipsWar.exe')
    run = False
