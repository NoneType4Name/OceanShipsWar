...
scrn = pygame.display.set_mode(size, pygame.RESIZABLE, pygame.SRCALPHA)
...
def re_theme():
    ...
    if theme:
        ...
        StartLoadProgress = ProgressBar((size[0]//2-size[0]*0.3,size[1]*0.6,size[0]*0.3,size[1]*0.1),LINES,(0,255,0),0)
        StartLoadLabel = label(
            (size[0]//2-size[0]*0.2/2,size[1]*0.6,size[0]*0.2,size[1]*0.05), '0 %', (0, 0, 0, 0),
            (0, 255, 0), True)
        StartLoadLabel2 = label(
            (size[0]//2-size[0]*0.2/2,size[1]*0.65,size[0]*0.2,size[1]*0.05), '', (0, 0, 0, 0),
            (0, 255, 0), True)
        Update = button((-1, size[1] - bsize // 2, bsize * 2, bsize // 2), str(GameLanguage['version']),
                                (177, 220, 237),
        ...
        RoomQuit = button((size[0] - bsize + 1, -1, bsize, bsize),
                                  ...
                                  (127, 127, 127), (0, 0, 0))
        EscButton = button((-1, -1, (bsize + bsize * 0.5) // 2, bsize // 2), 'ESC', (177, 220, 237),
                                   (255, 255, 255), (64, 64, 64), (255, 255, 255), (127, 127, 127), (0, 0, 0))
        ButtonAdm = button(
        ...
        Update = button((-1, size[1] - bsize // 2, bsize * 2, bsize // 2), str(GameLanguage['version']),
                                ...
                                (0, 0, 0),
                                (255, 255, 255), (255, 255, 255), (127, 127, 127), (0, 0, 0))
        EscButton = button((-1, -1, (bsize + bsize * 0.5) // 2, bsize // 2), 'ESC', (255, 255, 255), (0, 0, 0),
                                   (255, 255, 255), (255, 255, 255), (127, 127, 127), (0, 0, 0))
        ButtonClient = button(
            (size[0] / 2 - (size[0] * 0.16) / 2, size[1] * 0.5 - size[1] * 0.1, size[0] * 0.16,
             size[1] * 0.1),
...
def draw_ship_count(ships_count, max_ships=[SOLO,DUO,TRIO,QUADRO]):
    ...
    for type_ship in range(len(ships_count)):
        counter_block_num = 0
        # print((max_ships[type_ship] * (type_ship + 1)) * 2 - 1)
...
def re_settings_button():
    ...
    class setting_element:
        def __init__(self, type_el, name, trigger=None, addictions=None):
            self.type = type_el
            self.name = name
            self.trigger = trigger
            self.addict = addictions
    set_of_settings = {
        'up margin': 0,
        'down margin': 1.9,
...
while run:
    ...
    if pygame.display.get_window_size()[0] != size[0] or pygame.display.get_window_size()[1] != size[1]:
        ...
        screen = pygame.Surface(size, pygame.RESIZABLE, pygame.SRCALPHA)
        scrn = pygame.display.set_mode(size, pygame.RESIZABLE, pygame.SRCALPHA)
        bsize = size[0] // 27.428571428571427
        ships_wh = int(bsize // 7)
        left_margin, upper_margin = (size[0] // 2 - bsize * 5), (size[1] // 2 - bsize * 5)
    ...
    if not FineLoadGame:
        ...
        if ButtonTheme.isCollide() and mouse_left_press:
            ...
            room = False
            re_settings_button()
            ERRORS.append('Настройки пока работают не все.\tПростите.')
